...
public class PewPew_easy {
	public PewPew_easy() {
		JFrame frame = new JFrame();
		frame = new JFrame("Pew pew");
		frame.setSize(600, 600);
		frame.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		frame.add(new Game_easy(frame.getSize()));
		frame.setVisible(true);
	}

public class Game_easy extends JPanel implements ActionListener, Runnable, KeyListener {
    ...
    public Game_easy(Dimension dimension) {
    ...
    protected void paintComponent(Graphics g) {
        ...
        if (gameOn == true) {
            ...
            for (int i = 0; i < b.length; i++) {
                for (int j = 0; j < b[0].length; j++) {
                    if (anyBallVisible && b[i][j].getY() <= 450) {
                        // count time
        ...
        if (!gameOn) {
            ...
            if (allBallsNotVisible) {
                ...
                g2.drawString("You Won, Score: "+score, 200, 300);

                g2.setColor(Color.pink);
                g2.setFont(new Font("serif", Font.BOLD, 20));
                g2.drawString("Press (Enter) to Restart", 200, 350);
            }
            else {
                for (int i = 0; i < b.length; i++) {
                    for (int j = 0; j < b[0].length; j++) {
